...
namespace fig {
WorldLayer::WorldLayer(Window* w, Camera* c, glm::mat4& view,
                       glm::mat4& projection,
...
void WorldLayer::init() {
  ...
  _projection =
      glm::perspective(glm::radians(_camera->fov()),
                       _window->width() / _window->height(), 0.01f, 1000.f);
  _light = std::make_unique<Light>(glm::vec3(1.2f, 0.0f, 5.0f), *_camera, _view,
                                   _projection);

  _colorShader = std::make_unique<PhongShader>(
      _light.get(), *_camera, _view, _projection,
