...
namespace fig {
QtWindow::QtWindow(fig::Window::Param param, EventFabric* eventFabric,
                   std::function<void(void)> appInitFn,
                   std::function<void(void)> appTickFn)
    : fig::Window(param), _appInitFn(appInitFn), _appTickFn(appTickFn) {
  ...
  QObject::connect(this, &OpenGLWindow::addToLog, log,
...
void QtWindow::initialize() { _appInitFn(); }

float QtWindow::width() const { return size().width(); }

float QtWindow::height() const { return size().height(); }
...
void QtWindow::show() { _context->swapBuffers(this); }
...
void addControl(QGridLayout* l, int row, QString label, float value,
